				  	...
				  	<c:choose>
				  		...
				  		<c:otherwise>
				  			<c:forEach var="jar" items="${jarList }">
							  	...
							  	<span class="title"><a href="${pageContext.request.contextPath}/jar/${jar.uuid }.html" target="_blank" title="${jar.noTagName }.jar下载">${jar.name }</a></span>
							   </li>
				  			</c:forEach>
				  		</c:otherwise>
				  	</c:choose>
